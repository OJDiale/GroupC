...
export default function MapPage(): React.JSX.Element {
  ...
  async function runCheck() {
    ...
    try {
      ...
      // Log this AI Safe Path calculation as a trip on the user's profile.
      const token = localStorage.getItem("token");
      if (token && locationSearched.name) {
        try {
          ...
          const startLocationName = startData.results?.[0]?.formatted || "Current location";
          await logUserDestination({ startLocation: startLocationName, endLocation: locationSearched.name }, token);
        } catch (logErr) {
          console.error("Failed to log AI Safe Path trip:", logErr);
        }
      }
    } catch (err) {
