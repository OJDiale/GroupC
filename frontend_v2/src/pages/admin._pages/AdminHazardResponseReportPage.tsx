...
interface AdminHazardResponseReportPageProps {
  /** True when rendered inside the admin sidebar layout — skip AdminShell. */
  embedded?: boolean;
}

export default function AdminHazardResponseReportPage({ embedded }: AdminHazardResponseReportPageProps = {}) {
          ...
          <table className="w-full border-collapse text-left text-xs min-w-[720px]">
            <thead className="bg-brand-bg">
              <tr className="text-[10px] uppercase tracking-wide text-brand-muted">
                ...
                <th className="p-2">Resolved By</th>
            ...
            <tbody className="divide-y divide-brand-border">
              ...
              {responses.map((r) => (
                <tr key={r.logId} className="hover:bg-brand-bg/60">
                  <td className="p-2 font-mono">{r.logId}</td>
                  <td className="p-2 font-mono">#{r.hazardId}</td>
                  <td className="p-2 capitalize">{r.hazardType.replace(/_/g, ' ')}</td>
                  <td className="p-2 font-semibold">{r.resolvedByUsername}</td>
                  <td className="p-2">
  ...
  if (embedded) {
    return (
      <div className="space-y-4">
        <p className="text-brand-muted text-sm">Audit trail of every hazard status change — who resolved (or reopened) what, and when.</p>
        {content}
      </div>
    );
  }
